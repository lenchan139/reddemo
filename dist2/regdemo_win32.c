...
static BOOL is_nt_windows(void) {
...
static void add_value_item(HWND listBox, ValueItem *item, HFONT font, int nameColumnWidth) {
    wchar_t row[4096];
    LRESULT index;
    ...
    StringCchPrintfW(row, ARRAYSIZE(row), L"%-*ls%ls", nameColumnWidth, item->displayName, item->displayValue);
...
static void enumerate_values(AppState *app, const TreeItem *treeItem) {
    ...
    if ((int)maxValueName + 1 > nameColumnWidth) {
        nameColumnWidth = (int)maxValueName + 1;
    }
    ...
    while (1) {
...

        {
            ...
            if (item != NULL) {
                add_value_item(app->valueList, item, app->monoFont, nameColumnWidth);
            }
